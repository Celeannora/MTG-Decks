...
def run_query(
    ...
    """
    script = repo_root / "scripts" / "search_cards.py"
...
def generate_session_file(
    ...
    lines = [
        ...
        "## Pre-Flight Checks",
        "",
        "- [ ] `cards_by_category/` is present and populated",
        "- [ ] `scripts/search_cards.py` runs without errors",
        "- [ ] `scripts/validate_decklist.py` runs without errors",
        "",
        "---",
        "",
    ...
    lines += [
        ...
        "",
        "```",
        "$ python scripts/search_cards.py --type <type> --colors <colors> [flags...]",
        "",
        "(paste output here)",
        "```",
        "",
        ...
        "",
        "```bash",
        f"python scripts/validate_decklist.py Decks/{deck_date}_{sanitize_folder_name(deck_name)}/decklist.txt --verbose",
        "```",
        "",
        "## Validation Output",
        "",
        ...
        "---",
        "",
        f"_Generated by `scripts/generate_deck_scaffold.py` on {deck_date}_",
        "",
    ]
...
def main() -> None:
    ...
    # Validate cards_by_category exists
    cards_dir = repo_root / "cards_by_category"
    if not cards_dir.exists() and not args.skip_queries:
        print("ERROR: cards_by_category/ not found.", file=sys.stderr)
        print("Run 'python scripts/fetch_and_categorize_cards.py' first.", file=sys.stderr)
        sys.exit(2)

    deck_date = args.date or date.today().isoformat()
    folder_name = f"{deck_date}_{sanitize_folder_name(args.name)}"
    output_base = Path(args.output_dir) if args.output_dir else repo_root / "Decks"
    ...
    analysis_template = repo_root / ".github" / "DECK_TEMPLATE" / "analysis.md"
    ...
    sb_template = repo_root / ".github" / "DECK_TEMPLATE" / "sideboard_guide.md"
    ...
    print(f"  4. Run: python scripts/validate_decklist.py {display_decklist}")
